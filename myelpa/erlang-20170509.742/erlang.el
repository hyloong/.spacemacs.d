...
(defun inferior-erlang-compile-outdir ()
  (setq buffer-dir (directory-file-name
                    (file-name-directory (erlang-local-buffer-file-name))))
  (setq parent-dir (directory-file-name
                    (file-name-directory buffer-dir)))
  (while parent-dir
    (setq pdir (file-name-nondirectory parent-dir))
    ;; first if
    (if (string= pdir "src") ;; src
        (progn
          (setq pp (directory-file-name (file-name-directory parent-dir)))
          (setq parent-dir nil)
          (setq ebin-dir (concat (file-name-as-directory pp) "ebin")))
      ;; second if
      (if (string= pdir "") ;; ""
          (progn 
            (setq parent-dir nil)
            (setq ebin-dir (file-name-as-directory buffer-dir)))
        ;; last 
        (setq parent-dir (directory-file-name (file-name-directory parent-dir)))
        )))
  (file-name-as-directory ebin-dir) )
